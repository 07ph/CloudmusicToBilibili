// ==UserScript==
// @name           BiliBili批量收藏工具
// @description    B站视频批量收藏工具，支持导入JSON文件和粘贴链接
// @version        0.7.0
// @author         Yiero & 定制
...
const api_removeVideoFromFavorite = async (videoId, favoriteId) => {
  const epId = await getVideoEpId();
  const formData = {
    rid: videoId,
    type: epId ? "42" : "2",
    del_media_ids: favoriteId,
    csrf: requestConfig.csrf
  };
  return xhrRequest(
    "/x/v3/fav/resource/deal",
...
const api_getVideoFavorites = async (aid) => {
  const res = await gmRequest('https://api.bilibili.com/x/v3/fav/resource/list', 'GET', {
    rid: aid,
    type: '2'
  });
  if (res.code !== 0) {
...
class Favourites {
  ...
  async addVideo(videoAvId) {
    ...
    try {
      ...
      const successfullyAdd = res.data.success_num > 0 || res.code === 0;
      if (!successfullyAdd) {
        console.error(res.data.toast_msg || '收藏失败');
        return false;
      }
  ...
  async createNew() {
    ...
    // 如果还没有收藏夹，创建第一个时不添加数字
    if (this.readFavouriteList.length === 0) {
      await api_createFavorites(this.readFavouriteTitle);
    } else {
      // 有收藏夹时，添加数字编号
...
// 批量收藏视频
const batchCollectVideos = async (content, folderTitle) => {
  try {
    // 初始化收藏夹
    await favourites.init();
    
    // 设置收藏夹标题
    const originalTitle = favourites.readFavouriteTitle;
    favourites.readFavouriteTitle = folderTitle;
    // 重新获取收藏夹列表，确保使用新的收藏夹名称
    favourites.getRead(true);
    ...
    try {
      ...
      if (jsonData.songs && Array.isArray(jsonData.songs)) {
        links = jsonData.songs
          .map(song => song.bilibili_url)
          .filter(url => url && url.includes('bilibili.com/video/'));
      }
    } catch (e) {
      // 如果不是JSON，按文本处理
      links = content.trim().split('\n').filter(link => link.includes('bilibili.com/video/'));
    }
    ...
    if (total === 0) {
      Message.error('未找到B站链接');
      return;
    }
    
    Message.info(`开始批量收藏，共 ${total} 个视频到收藏夹「${folderTitle}」`);
    
    for (let i = 0; i < links.length; i++) {
      const link = links[i].trim();
      try {
        // 提取BV号
        const match = link.match(/BV[0-9A-Za-z]+/);
        if (!match) {
          console.error(`无法提取BV号: ${link}`);
          failed++;
          continue;
        }
        ...
        // 直接收藏到指定收藏夹，不管是否已在其他收藏夹中
        if (!favourites.getRead().length) {
          await favourites.createNew();
        }
        const collectResult = await favourites.addVideo(aid);
        
        // 显示进度
        const progress = Math.round((i + 1) / total * 100);
        Message.info(`收藏进度: ${progress}% (${i + 1}/${total})`);
    ...
    // 恢复原始收藏夹标题
    favourites.readFavouriteTitle = originalTitle;
    
    // 显示结果
    Message.success(`批量收藏完成！成功: ${success}, 失败: ${failed}`);
  } catch (error) {
    console.error('批量收藏出错:', error);
    Message.error('批量收藏过程出错');
  }
};

// 导入JSON文件
const importJsonFile = async (folderTitle) => {
  try {
    // 创建文件输入元素
    ...
    input.onchange = async (e) => {
      ...
      reader.onload = async (event) => {
        try {
          ...
          await batchCollectVideos(content, folderTitle);
        } catch (error) {
    ...
    console.error('导入JSON文件出错:', error);
    Message.error('导入JSON文件过程出错');
  }
};

// 检查视频是否已收藏（带aid参数）
const api_isFavorVideoById = async (aid) => {
  const res = await gmRequest('https://api.bilibili.com/x/v2/fav/video/favoured', 'GET', {
    aid
  });
  if (res.code !== 0) {
...
const createFloatingPanel = () => {
  ...
  floatingPanel.id = 'bili-collector-panel';
  ...
  floatingPanel.innerHTML = `
    <div style="background: linear-gradient(135deg, #00a1d6 0%, #008ebf 100%); padding: 16px; color: white; display: flex; justify-content: space-between; align-items: center;">
      <div style="font-size: 18px; font-weight: 600;">B站批量收藏工具</div>
      <button id="bili-collector-close" style="background: rgba(255, 255, 255, 0.2); color: white; border: none; border-radius: 4px; padding: 4px 8px; font-size: 12px; cursor: pointer;">关闭</button>
    </div>
    <div style="padding: 16px;">
      <div style="margin-bottom: 16px;">
        <label style="display: block; margin-bottom: 8px; font-weight: 500; color: #333;">收藏夹名称</label>
        <input type="text" id="bili-collector-folder" style="width: 100%; padding: 10px; border: 2px solid #e0e0e0; border-radius: 8px; font-size: 14px; box-sizing: border-box; outline: none; transition: border-color 0.3s;" value="${favoriteTitleStorage.get()}">
      </div>
      <div style="margin-bottom: 16px;">
        <label style="display: block; margin-bottom: 8px; font-weight: 500; color: #333;">粘贴B站链接（每行一个）</label>
        <textarea id="bili-collector-links" style="width: 100%; height: 120px; padding: 10px; border: 2px solid #e0e0e0; border-radius: 8px; font-size: 13px; box-sizing: border-box; outline: none; resize: vertical; font-family: monospace;"></textarea>
      </div>
      <div style="display: flex; gap: 10px; margin-bottom: 16px;">
        <button id="bili-collector-paste" style="flex: 1; padding: 12px; background: #00a1d6; color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 500; cursor: pointer; transition: background 0.3s;">粘贴内容收藏</button>
        <button id="bili-collector-import" style="flex: 1; padding: 12px; background: #6b7280; color: white; border: none; border-radius: 8px; font-size: 14px; font-weight: 500; cursor: pointer; transition: background 0.3s;">导入JSON文件</button>
      </div>
      <div id="bili-collector-status" style="padding: 12px; background: #f3f4f6; border-radius: 8px; font-size: 13px; color: #666; min-height: 40px; display: flex; align-items: center;">
        准备就绪
      </div>
    </div>
  `;
  ...
  const folderInput = document.getElementById('bili-collector-folder');
  folderInput.onchange = () => {
    favoriteTitleStorage.set(folderInput.value);
  };

  document.getElementById('bili-collector-close').onclick = () => {
    floatingPanel.style.display = 'none';
  };

  document.getElementById('bili-collector-paste').onclick = async () => {
    const folderTitle = document.getElementById('bili-collector-folder').value;
    const content = document.getElementById('bili-collector-links').value;
    if (!folderTitle) {
      updateStatus('请输入收藏夹名称！', 'error');
      return;
    }
    if (!content.trim()) {
      updateStatus('请粘贴B站链接！', 'error');
      return;
    }
    await batchCollectVideosWithStatus(content, folderTitle);
  };

  document.getElementById('bili-collector-import').onclick = async () => {
    const folderTitle = document.getElementById('bili-collector-folder').value;
    if (!folderTitle) {
      updateStatus('请输入收藏夹名称！', 'error');
      return;
    }
    await importJsonFileWithStatus(folderTitle);
  };
};
...
const updateStatus = (message, type = 'info') => {
  const statusEl = document.getElementById('bili-collector-status');
  if (!statusEl) return;
  ...
  statusEl.style.background = backgrounds[type] || backgrounds.info;
};

const batchCollectVideosWithStatus = async (content, folderTitle) => {
  try {
    updateStatus('正在初始化收藏夹...', 'info');
    await favourites.init();
    const originalTitle = favourites.readFavouriteTitle;
    favourites.readFavouriteTitle = folderTitle;
    // 重新获取收藏夹列表，确保使用新的收藏夹名称
    ...
    const total = links.length;
    if (total === 0) {
      updateStatus('未找到有效的B站链接', 'error');
      return;
    }

    updateStatus(`开始批量收藏，共 ${total} 个视频...`, 'info');
    let success = 0;
    let failed = 0;
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      try {
        ...
        const progress = Math.round((i + 1) / total * 100);
        updateStatus(`收藏进度: ${progress}% (${i + 1}/${total}) - 成功:${success} 失败:${failed}`, 'info');
        await sleep(2000);
      } catch (error) {
        console.error('收藏出错:', link, error);
        failed++;
        await sleep(1000);
      }
    }
    ...
    favourites.readFavouriteTitle = originalTitle;
    updateStatus(`批量收藏完成！成功: ${success}, 失败: ${failed}`, 'success');
  } catch (error) {
    console.error('批量收藏出错:', error);
    updateStatus('批量收藏过程出错', 'error');
  }
};

const importJsonFileWithStatus = async (folderTitle) => {
  try {
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = '.json';
    ...
    input.onchange = async (e) => {
      ...
      reader.onload = async (event) => {
        try {
          ...
          updateStatus('解析JSON文件失败，请检查文件格式', 'error');
        }
      };
    ...
    updateStatus('导入JSON文件过程出错', 'error');
  }
};
